...
|4.|**EFFECTS**|If a new borrow order is processed in a matching round, Backend constructs **L2 admin request: AuctionStart**|[Request Format](#requests)|Y|The circuit analyzes the request with its format|
...
|10.|**EFFECTS**|Backend constructs **L2 admin request: AuctionMatch**|[Request Format](#requests)|Y|The circuit analyzes the request with its format|
...
|14.|**EFFECTS**|If the borrow order has no more matches in this round, deduct the collateral amount in the matched orders from the total locked collateral amount||Y||
|15.|**EFFECTS**|If the borrow order has no more matches in this round, the Backend constructs **L2 admin request: AuctionEnd**|[Request Format](#requests)|Y|The circuit analyzes the request with its format|
|16.|**EFFECTS**|If the borrow order is completed, return the remaining locked amount in the borrow order||Y||
